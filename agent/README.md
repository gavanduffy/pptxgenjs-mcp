...
- NEW: **Markdown Import**: Generate slides from Markdown with headings, lists, images, code blocks, quotes, and more

## Installation

### Smithery Deployment (Recommended)

This server is designed for deployment via [Smithery](https://smithery.ai), providing fully managed hosting with automatic scaling:

1. **Prerequisites**:
   - Cloudflare account with Workers and R2 enabled
   - Cloudflare Account ID (found in Workers dashboard)

2. **Deploy via Smithery**:
   ```bash
   # Validate configuration
   npm run smithery:validate
   
   # Deploy to Cloudflare via Smithery
   npm run smithery:deploy
   ```

3. **Configure Environment**:
   Set the following secrets in your Cloudflare Workers dashboard:
   ```bash
   wrangler secret put CLOUDFLARE_ACCOUNT_ID
   # Optional: Set custom domain for R2 bucket
...
4. **Create R2 Bucket**:
   ...
   wrangler r2 bucket create pptx-preview  # For staging
   ```

5. **Access Your Server**:
   - Production: `https://pptxgenjs-mcp.workers.dev/mcp`
   - Custom domain: Configure in `wrangler.toml` routes section
...
#### `import_markdown_presentation`
Create slides from Markdown text with high-fidelity mapping to slide content. Supports:

- H1/H2 headings → slide titles or section headers (configurable split level)
- Paragraphs and inline code → text boxes and styled runs
- Nested ordered/unordered lists → bulleted lists with levels
- Images (`![](url)`) → inserted images with optional caption
- Code blocks (fenced) → monospaced text blocks
- Blockquotes and horizontal rules → styled paragraphs and spacing

Parameters:
- `presentationId`: Presentation ID
- `markdown`: Markdown source string
- `options` (optional):
  - `splitLevel` (1 or 2): Heading level that starts a new slide (default: 2)
  - `titleSlide` (boolean): Create a title slide from the first H1 (default: true)
  - `defaults`: `{ titleFontSize, bodyFontSize, codeFontFace, codeFontSize, textColor }`
  - `layout`: `{ marginX, marginY, contentWidth }` in inches
  - `image`: `{ defaultWidth, defaultHeight }` in inches
